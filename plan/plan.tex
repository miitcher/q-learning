...
so group must write own visual representation of world and agents.
Box2d uses float numbers in simulation, and that causes minor inaccuracies in world.
Main features of box2d that are going to be used in project are: bodies, joints, gravity and friction.
Joints are rotatable and provide core for agent to move in world. 
If joint is forced to turn where it can not turn, it will not turn.
Gravity and friction are used to make project more realistic.
